...
from util.plotUtil import visualization_test
    ...
    with torch.no_grad():
        for src, target, rotation_ab, translation_ab, rotation_ba, translation_ba, euler_ab, euler_ba, label in tqdm(
            ...
            if args.no_cuda:
                src = src.cpu()
                target = target.cpu()
                rotation_ab = rotation_ab.cpu()
                translation_ab = translation_ab.cpu()
                rotation_ba = rotation_ba.cpu()
                translation_ba = translation_ba.cpu()
            else:
                src = src.cuda()
                target = target.cuda()
            ...
            srcInit,src, rotation_ab_pred, translation_ab_pred, rotation_ba_pred, translation_ba_pred = net(src, target)
            ...
            # savePlt(args, target, src, name='corrpose')
            ...
            ###########################
            if args.no_cuda:
                identity = torch.eye(3).cpu().unsqueeze(0).repeat(batch_size, 1, 1)
            else:
...
def testICP(args, net, test_loader, boardio, textio):
    ...
    textio.cprint('A--------->B')
    textio.cprint('EPOCH:: %d, Loss: %f, Cycle Loss: %f, MSE: %f, RMSE: %f, MAE: %f, rot_MSE: %f, rot_RMSE: %f,  rot_MAE: %f,trans_MSE: %f, trans_RMSE: %f, trans_MAE: %f'
...
if __name__ == "__main__":
    ...
    # print(R_ab-rotation_ab_pred.cpu().numpy().squeeze())
    # plot3d(src,src_corr)
    # plot3d(target,src_corr)
